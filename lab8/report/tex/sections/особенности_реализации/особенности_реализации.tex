...
функция saveContacts(contacts):
    ...
    // заголовок файла
    stream << FILE_MAGIC   // 0x50484F4E ("PHON")
    stream << FILE_VERSION // 1
...
функция saveContactsYaml(contacts):
    ...
    для каждого contact в contacts
        ...
        out << Key << "lastName" << Value << contact.lastName
        out << Key << "firstName" << Value << contact.firstName
        out << Key << "middleName" << Value << contact.middleName
        out << Key << "address" << Value << contact.address
        out << Key << "email" << Value << contact.email
        out << Key << "birthDate" << Value << contact.birthDate
        
        out << Key << "phones" << Value << BeginSeq
        для каждого phone в contact.phones
            out << BeginMap
            out << Key << "type" << Value << phone.typeString()
            out << Key << "number" << Value << phone.number
            out << EndMap
        конец цикла
        out << EndSeq
...
функция loadContactsYaml(contacts):
    ...
    для каждого node в root
        ...
        если node["lastName"] существует
            contact.lastName := node["lastName"].as<string>()
        если node["firstName"] существует
            contact.firstName := node["firstName"].as<string>()
        // ... остальные поля аналогично
        
        если node["phones"] существует и является списком
            для каждого phoneNode в node["phones"]
                phone := новый PhoneNumber()
                phone.type := разобрать_тип(phoneNode["type"])
                phone.number := phoneNode["number"].as<string>()
